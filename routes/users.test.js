...
describe("POST /users/:username/jobs/:id", function () {
  test("works for same user", async function () {
    const resp = await request(app)
      .post(`/users/testuser/jobs/${testJobIds[0]}`)
  ...
  test("not found for invalid job id", async function () {
    const resp = await request(app)
      .post(`/users/testuser/jobs/99999`)
      .set("authorization", `Bearer ${adminToken}`);
  ...
  test("works", async function () {
    const jobRes = await db.query(`SELECT id FROM jobs WHERE title='Software Engineer'`);
    console.log("Job ID:", jobRes.rows);
  
    const job = await Job.get(jobRes.rows[0].id); // Use actual ID from DB
    expect(job).toEqual({
      id: jobRes.rows[0].id,
